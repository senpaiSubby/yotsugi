...
*/
class OmbiMovies extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'movie',
            category: 'Media',
    ...
    async run(client, msg, args) {
        ...
        if (!role) {
            ...
            return channel.send(Utils.embed('yellow')
                .setTitle('Missing role [ requestmovie ]')
                .setDescription('I created a role called **requestmovie**. Assign this role to members to let them request movies!'));
        }
        // * ------------------ Config --------------------
        ...
        const outputMovie = (movie) => {
            ...
            if (movie.quality)
                e.addField('Quality', movie.quality, true);
            if (movie.requested)
        ...
        const getTMDbID = async (name) => {
            try {
                const response = await unirest_1.get(url_join_1.default(host, '/api/v1/Search/movie/', name)).headers({
        ...
        const requestMovie = async (movie) => {
            ...
            if (!movie.available && !movie.requested && !movie.approved) {
                try {
                    await unirest_1.post(url_join_1.default(host, '/api/v1/Request/movie/'))
                        .headers({
                        accept: 'application/json',
        ...
        if (results.length) {
            ...
            for (const movie of results) {
                try {
                    const response = await unirest_1.get(url_join_1.default(host, '/api/v1/Search/movie/info/', String(movie.id))).headers({ ApiKey: apiKey, accept: 'application/json' });
