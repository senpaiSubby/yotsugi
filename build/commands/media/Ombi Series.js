...
*/
class OmbiTV extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'tv',
            category: 'Media',
    ...
    async run(client, msg, args) {
        ...
        if (!role) {
            ...
            return msg.channel.send(embed('yellow')
                .setTitle('Missing role [ requesttv ]')
                .setDescription('I created a role called **requesttv**. Assign this role to members to let them request TV Shows!'));
        }
        // * ------------------ Config --------------------
        ...
        const outputTVShow = (show) => {
            ...
            if (show.available)
                e.addField('Available', '✅', true);
            if (show.quality)
                e.addField('Quality', show.quality, true);
            if (show.requested)
                e.addField('Requested', '✅', true);
            if (show.approved)
                e.addField('Approved', '✅', true);
            if (show.plexUrl)
                e.addField('Plex', `[Watch Now](${show.plexUrl})`, true);
            if (show.embyUrl)
                e.addField('Emby', `[Watch Now](${show.embyUrl})`, true);
            return e;
        };
        const getTVDBID = async (name) => {
            try {
                const response = await unirest_1.get(url_join_1.default(host, '/api/v1/Search/tv/', name)).headers({
        ...
        const requestTVShow = async (show) => {
            ...
            if (!show.available && !show.requested && !show.approved) {
                try {
                    await unirest_1.post(url_join_1.default(host, '/api/v1/Request/tv/'))
                        .headers({
        ...
        if (results) {
            ...
            for (const show of results) {
                try {
                    const response = await unirest_1.get(url_join_1.default(host, '/api/v1/Search/tv/info/', String(show.id))).headers({ ApiKey: apiKey, accept: 'application/json' });
