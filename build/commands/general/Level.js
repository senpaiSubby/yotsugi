        ...
        switch (args[0]) {
            case 'add': {
                if (await checkRole()) {
                    ...
                    if (!role)
                        return warningMessage(msg, `You must provide a role`);
                    if (!level)
                        return warningMessage(msg, `You must provide a level`);
                    // Get index of role level in config
                    let index = levelRoles.findIndex((i) => i.level === level);
                    // If level role doesnt exit then add it in
                    if (index === -1) {
                        levelRoles.push({ level, role });
                        index = levelRoles.findIndex((i) => i.level === level);
                    }
            ...
            case 'list': {
                if (levelRoles.length) {
                    const e = embed('green', 'question.png')
                        ...
                        .setDescription('Here are the roles you will be assigned according to your level. Chat often if you want to level up!');
