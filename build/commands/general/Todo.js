...
class Todo extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'todo',
            category: 'General',
    ...
    async run(client, msg, args, api) {
        ...
        switch (args[0]) {
            case 'add': {
                if (todos.length >= 10) {
                    if (api)
                        return 'You cannot have more than [ 10 ] todos!';
            ...
            case 'list': {
                if (!todos.length) {
                    ...
                    return channel.send(embed('yellow')
                ...
                const filter = (reaction, user) => {
                    ...
                    if (reactions.includes(name) && user.id === author.id)
                        return true;
                    if (name === '🛑' && user.id === author.id)
                        return true;
                };
            ...
            default:
                if (api)
                    return 'Valid options are [ add, list ]';
