...
class Routines extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'routine',
            category: 'Utils',
    ...
    async run(client, msg, args, api) {
        ...
        // * ------------------ Config --------------------
        const db = await database_1.default.models.GeneralConfig.findOne({ where: { id: client.config.ownerID } });
        ...
        switch (args[0]) {
            case 'rename': {
                ...
                if (routineIndex === -1) {
                    if (api)
                        return `Routine [ ${routineName} ] doesn't exist`;
                    return warningMessage(msg, `Routine [ ${routineName} ] doesn't exist`);
                }
                // Check for new name
                if (!newName) {
                    if (api)
                        return `Please specify the name for routine [ ${newName} ]`;
                ...
                if (api)
                    return `Renamed routine [ ${routineName} ] to [ ${newName} ]`;
            ...
            case 'enable': {
                ...
                if (status[0]) {
                    if (api) {
                        return `Command [ ${command + 1} ] [ ${commandListIndex[1]} ] in routine [ ${routineName} ] is already enabled`;
                    }
                    return warningMessage(msg, `Command [ ${command + 1} ] [ ${commandListIndex[1]} ] in routine [ ${routineName} ] is already enabled`);
                }
                ...
                return standardMessage(msg, `Enabled command  [ ${command + 1} ] [ ${commandListIndex[1]} ] in routine [ ${routineName} ]`);
            }
            case 'disable': {
                const routineName = args[1];
                const routineIndex = routines.findIndex((i) => i.name === routineName);
                ...
                if (!status[0]) {
                    if (api) {
                        return `Command [ ${command + 1} ] [ ${commandListIndex[1]} ] in routine [ ${routineName} ] is already disabled`;
                    }
                    return warningMessage(msg, `Command [ ${command + 1} ] [ ${commandListIndex[1]} ] in routine [ ${routineName} ] is already disabled`);
                }
                ...
                return standardMessage(msg, `Disabled command  [ ${command + 1} ][ ${commandListIndex[1]} ] in routine [ ${routineName} ]`);
            }
            case 'run': {
                const routineName = args[1];
                ...
                if (failedCommands.length) {
                    if (api)
                        return `Commands [ ${failedCommands.join(', ')} ] dont exist`;
                    return errorMessage(msg, `Commands [ ${failedCommands.join(', ')} ] dont exist`);
                }
                if (api)
                    return `Running routine [ ${routineName} ]`;
                break;
            }
            case 'list': {
                if (!routines.length) {
                    if (api)
                        return `There are no routines!`;
            ...
            case 'add': {
                ...
                if (!command) {
                    if (api)
                        return `Please specify the command to add to the routine`;
            ...
            case 'remove': {
                ...
                await db.update({ config: JSON.stringify(config) });
                if (api)
                    return `Removed routine [ ${routineName} ]`;
