...
class AutoRun extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'autorun',
            category: 'Utils',
    ...
    async run(client, msg, args, api) {
        ...
        switch (args[0]) {
            case 'changetime': {
                ...
                if (timeIndex === -1) {
                    if (api)
                        return `[ ${taskTime} ] autorun doesn't exist`;
                    return warningMessage(msg, `[ ${taskTime} ] autorun at doesn't exist`);
                }
                // Check for new name
                if (!newTime) {
                    if (api)
                        return `Please specify the time for autorun [ ${newTime} ]`;
                ...
                if (api)
                    return `Retimed [ ${taskTime} ] autorun to [ ${newTime} ]`;
            ...
            case 'enable': {
                ...
                if (status[0]) {
                    ...
                    return warningMessage(msg, `Command [ ${command + 1} ] [ ${commandListIndex[1]} ] in [ ${taskTime} ] autorun is already enabled`);
                }
                ...
                await standardMessage(msg, `Enabled command  [ ${command + 1} ] [ ${commandListIndex[1]} ] in [ ${taskTime} ] autorun`);
                return process.exit();
            }
            case 'disable': {
                const taskTime = args[1];
                ...
                if (!status[0]) {
                    if (api) {
                        return `Command [ ${command + 1} ] [ ${commandListIndex[1]} ] in [ ${taskTime} ] autorun is already disabled`;
                    }
                    return warningMessage(msg, `Command [ ${command + 1} ] [ ${commandListIndex[1]} ] in [ ${taskTime} ] autorun is already disabled`);
                }
                ...
                await standardMessage(msg, `Disabled command  [ ${command + 1} ][ ${commandListIndex[1]} ] in [ ${taskTime} ] autorun`);
                return process.exit();
            }
            case 'list': {
                if (!autorun.length) {
                    if (api)
                        return `There are no autorun!`;
            ...
            case 'add': {
                ...
                if (!command) {
                    if (api)
                        return `Please specify the command to add to the autorun`;
                ...
                if (autorun[index].commands.includes(command)) {
                    if (api)
                        return `[ ${taskTime} ] autorun already has command [ ${command} ]`;
                    return warningMessage(msg, `[ ${taskTime} ] autorun already has command [ ${command} ]`);
                }
                // Save changes
                await db.update({ config: JSON.stringify(config) });
                if (api)
                    return `Added command [ ${command} ] to [ ${taskTime} ] autorun`;
            ...
            case 'remove': {
                ...
                if (args[2]) {
                    ...
                    await db.update({ config: JSON.stringify(config) });
                    if (api)
                        return `Removed command [ ${command} ] from [ ${taskTime} ] autorun`;
                ...
                await db.update({ config: JSON.stringify(config) });
                if (api)
                    return `Removed [ ${taskTime} ] autorun`;
