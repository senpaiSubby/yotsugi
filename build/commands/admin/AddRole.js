 ...
 * Add roles to users
 */
class AddRole extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'addrole',
            category: 'Admin',
    ...
    async run(client, msg, args) {
        ...
        // Then notify in chat
        try {
            return rMember.send(embed('green').setDescription(`**You have been given the role [ ${gRole.name} ] in [ ${msg.guild.name} ]**`));
        }
        catch (e) {
            return warningMessage(msg, `<@${rMember.id}> They have been given the role ${gRole.name} \
