...
class Sengled extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'lights',
            category: 'Smart Home',
    ...
    async run(client, msg, args, api) {
        ...
        const login = async () => {
            try {
                const response = await unirest_1.post(`${baseUrl}/customer/remoteLogin.json`)
                ...
                if (api)
                    return `Failed to connect to Sengled`;
        ...
        const getDevices = async () => {
            try {
                const response = await unirest_1.post(`${baseUrl}/room/getUserRoomsDetail.json`).headers(headers);
                const data = response.body;
                const deviceList = [];
        ...
        const setLight = async (deviceID, deviceName, newState) => {
            try {
                await unirest_1.post(`${baseUrl}/device/deviceSetOnOff.json`)
                    .headers(headers)
                    .send({
                    deviceUuid: deviceID,
                ...
                if (api)
                    return `[ ${deviceName} ] light turned [ ${state} ]`;
        ...
        const setBrightness = async (deviceUuid, deviceName, newBrightness) => {
            ...
            try {
                ...
                const response = await unirest_1.post(`${baseUrl}/device/deviceSetBrightness.json`)
                    .headers(headers)
        ...
        if (devices) {
            switch (args[0]) {
                case 'id': {
                    const x = await login();
                    return standardMessage(msg, `${x.jsessionid}`);
                }
                case 'list': {
                    if (api)
                        return devices;
