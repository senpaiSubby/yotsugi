...
class Tuya extends Command_1.Command {
    constructor(client) {
        super(client, {
            name: 'plug',
            category: 'Smart Home',
    ...
    async run(client, msg, args, api) {
        ...
        const listPlugs = async () => {
            try {
                ...
                await asyncForEach(tuyaDevices, async (d) => {
                    const device = new tuyapi_1.default({ id: d.id, key: d.key });
                    await device.find();
        ...
        const togglePlug = async (d) => {
            ...
            try {
                const device = new tuyapi_1.default({ id, key });
                await device.find();
                await device.connect();
                const currentStatus = await device.get();
