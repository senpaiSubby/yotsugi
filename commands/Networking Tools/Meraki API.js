...
class MerakiAPI extends Command {
  ...
  async run(msg, args, api) {
    // -------------------------- Setup --------------------------
    const { bytesToSize, addSpace, sortByKey } = this.client.utils
    const logger = this.client.logger

    // ------------------------- Config --------------------------

    const { serielNum, apiKey } = this.client.config.commands.meraki
      ...
      try {
        ...
        if (devices) {
          ...
          for (const device of devices) {
            ...
            sent = sent + device.usage.recv
            recv = recv + device.usage.sent
            // new device json
            const stats = {
              name: description,
              ip: ip,
