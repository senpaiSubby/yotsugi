    ...
    switch (args[0]) {
      case 'add': {
        ...
        if (!todo) {
          ...
          const m = (await warningMessage(msg, 'Todo cannot be empty!')) as NezukoMessage
      ...
      case 'list': {
        ...
        const m = (await channel.send(e)) as NezukoMessage
        ...
        collector.on('collect', async (a) => {
          ...
          todos.forEach((i: any, ind: any) => newEmbed.addField(`[ ${ind + 1} ]`, `${i}`, true))
          await m.edit(newEmbed)
          await asyncForEach(todos, async (i: any, ind: any) => {
